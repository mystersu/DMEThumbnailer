...
unsigned long long NSStringMurmurHash64B (NSString* str);
...
{
    unsigned long long pathHash = NSStringMurmurHash64B(aPath);
    NSString* pathExt = [aPath pathExtension];
    
    if([aPrefix isEqualToString:@""]) {
        return [NSString stringWithFormat:@"%@/%@/%llu.%@", [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0], @"Thumbs", pathHash, pathExt];
    }
    else{
        return [NSString stringWithFormat:@"%@/%@/%@-%llu.%@", [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0], @"Thumbs", aPrefix, pathHash, pathExt];
    }
}
...
unsigned long long NSStringMurmurHash64B (NSString* str)
{
    ...
    return MurmurHash64B([strData bytes], [strData length], 17021105);
}
